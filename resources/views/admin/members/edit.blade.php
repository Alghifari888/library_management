<x-admin-layout>
    ...
    <form method="POST" action="{{ route('admin.members.update', $member) }}" enctype="multipart/form-data" class="mt-6 space-y-6">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <x-text-input id="email" name="email" type="email" class="mt-1 block w-full" :value="old('email', $member->email)" required />
            <x-input-error class="mt-2" :messages="$errors->get('email')" />
        </div>

        {{-- Password (Opsional) --}}
        <div>
            <x-input-label for="password" :value="__('Password Baru (Opsional, isi jika ingin mengubah)')" />
        ...
        <div>
            ...
            <x-text-input id="nis_nim" name="nis_nim" type="text" class="mt-1 block w-full" :value="old('nis_nim', $member->nis_nim)" />
        ...
        <div>
            ...
            <textarea id="address" name="address" rows="3" class="mt-1 block w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm">{{ old('address', $member->address) }}</textarea>
        ...
        {{-- Foto Profil --}}
        <div>
            <x-input-label for="profile_photo" :value="__('Ganti Foto Profil (Opsional)')" />
            @if ($member->profile_photo_path)
                <div class="mt-2 mb-2">
                    <p class="text-sm text-gray-600 dark:text-gray-400">Foto Saat Ini:</p>
                    <img src="{{ Storage::url($member->profile_photo_path) }}" alt="Foto {{ $member->name }}" class="h-20 w-20 object-cover rounded-full mt-1">
                </div>
            @else
                <p class="mt-2 mb-2 text-sm text-gray-600 dark:text-gray-400">Tidak ada foto profil saat ini.</p>
            @endif
            ...
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-300" id="file_input_help_profile_edit">Biarkan kosong jika tidak ingin mengganti. PNG, JPG, GIF, WEBP (MAX. 2MB).</p>
            <x-input-error class="mt-2" :messages="$errors->get('profile_photo')" />
        </div>

        <div class="flex items-center gap-4">
            <x-primary-button>{{ __('Update Anggota') }}</x-primary-button>
